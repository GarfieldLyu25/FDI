...
class PanelDataDisplay(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.SetSizer(self.buidUISizer())
        self.over_threshold_count = 0  # 连续超限计数器
    ...
    def _addDisplayError(self, sizer, name, flag):
        # 添加标签
        sizer.Add(wx.StaticText(self, -1, str(name)), 0, flag)
        # label = wx.StaticText(self, -1, str(name), 0, flag)
        # sizer.Add(label, 0, flag)
        # 使用 StaticText 显示错误状态
        error_label = gizmos.LEDNumberCtrl(self, -1, size=(70, 30), style=gizmos.LED_ALIGN_CENTER)
        error_label.SetValue('----')
        error_label.SetForegroundColour(wx.GREEN)  # 设置颜色（绿色表示无错误）
        self.error_labels.append(error_label)  # 单独存储错误状态控件
        sizer.Add(error_label)

    def _addDisplayErrorTest(self, sizer, name, flag):
        # 添加标签
        label = wx.StaticText(self, -1, str(name))
        sizer.Add(label, 0, flag)
        # 使用 StaticText 显示错误状态
        error_label = wx.StaticText(self, -1, 'NO ERROR')
        error_label.SetForegroundColour(wx.GREEN)  # 初始状态为绿色
        error_label.SetFont(wx.Font(12, wx.DEFAULT, wx.NORMAL, wx.BOLD))
        self.error_labels.append(error_label)  # 存储到独立列表
        # 添加控件到布局
        sizer.Add(error_label, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_LEFT | wx.RIGHT, 10)
    ...
    def _buildRightUISizer(self):
        ...
        self._addDisplayErrorTest(sizer, 'error: ', flagsL)
    ...
    def updateLedData(self):
        if gv.idataMgr:
            ...
            self.dataleds[25].SetValue(str(220))
            self.dataleds[26].SetValue(str(dataDict['load2'][1]))

    def check_voltage_threshold(self):
        self.has_error = int(self.dataleds[21].GetValue()) > 100
        self.update_error_statusTest()
        self.getRailwayState()

    def getRailwayState(self):
        # 获取负载1的电压值（假设索引2对应电压，单位kV）
        load1_voltage = int(self.dataleds[21].GetValue())
        # 判断是否超过阈值
        if load1_voltage > 100:
            log_msg = f"MU measurement value exceeds the range voltage value (100kV). Current Voltage: {load1_voltage}kV"
            gv.iMainFrame.updateTFDetail(log_msg)

            # 增加连续超限计数器
            self.over_threshold_count += 1

            # 判断是否连续三次超限
            if self.over_threshold_count >= 3:
                # 1. 打印注入成功日志
                success_msg = "active safety mechanism:load1 out cut off!"
                gv.iMainFrame.updateTFDetail(success_msg)
                # 2. 调用断路函数
                gv.iMapPanel.stopPLC()
                # 3. 重置计数器
    ...
    def update_error_statusTest(self):
        for error_label in self.error_labels:
            error_label.SetLabel("ERROR" if self.has_error else "NO ERROR")
            error_label.SetForegroundColour(wx.RED if self.has_error else wx.GREEN)

    def update_error_status(self):
        for error_label in self.error_labels:
            error_label.SetValue("0000" if self.has_error else "----")
            error_label.SetForegroundColour(wx.RED if self.has_error else wx.GREEN)
